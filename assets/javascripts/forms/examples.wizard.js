	...
	*/
	...
	$('#w4').bootstrapWizard({
		...
		onNext: function( tab, navigation, index, newindex ) {
			...
			if (index==1){ // Validación, selección de Solicitud

			}
			if (index==2){ //Validación, Fotos Iniciales
				if ($("#tImagen1").val()==''){
					return false;
				}
			}
		...
		onTabClick: function( tab, navigation, index, newindex ) {
			//alert("onTabClick tab="+tab.attr('id')+" navigation:"+navigation.attr('id')+" index:"+index+" newindex:"+newindex);
			if ( newindex == index + 1 ) {
				return this.onNext( tab, navigation, index, newindex);
			} else if ( newindex > index + 1 ) {
				return false;
		...
		onTabChange: function( tab, navigation, index, newindex ) {
			//alert("onTabChange tab="+tab.attr('id')+" navigation:"+navigation.attr('id')+" index:"+index+" newindex:"+newindex);
			var $total = navigation.find('li').size() - 1;
			$w4finish[ newindex != $total ? 'addClass' : 'removeClass' ]( 'hidden' );
			$('#w4').find(this.nextSelector)[ newindex == $total ? 'addClass' : 'removeClass' ]( 'hidden' );
		},
		onTabShow: function( tab, navigation, index ) {
			//alert("onTabShow tab="+tab.attr('id')+" navigation:"+navigation.attr('id')+" index:"+index);
			var $total = navigation.find('li').length - 1;
			var $current = index;
			var $percent = Math.floor(( $current / $total ) * 100);
